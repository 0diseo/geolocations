module GeolocationsService
  ...
  def self.get_location_from_ip_ipstack(ip_address)
    url = "https://api.ipstack.com/#{ip_address}?access_key=#{Rails.application.credentials.ipstack_api_key}"
  ...
  def self.get_address_from_geolocation(latitude, longitude)
    url = "https://us1.locationiq.com/v1/reverse?lat=#{latitude}&lon=#{longitude}&format=json&key=#{Rails.application.credentials.locationiq_api_key}"
    response = RestClient.get(url)
    results = JSON.parse(response.to_str)
    results["address"]
  end
